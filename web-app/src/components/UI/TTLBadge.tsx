...
export function TTLBadge({ createdAtMs, ttlMs }: { createdAtMs: number; ttlMs: number }) {
  ...
  return <span className={`inline-flex items-center whitespace-nowrap tabular-nums px-2 py-0.5 rounded text-[10px] font-semibold border leading-4 ${color}`} title={`TTL: ${mins} dk`}>TTL {mins}dk</span>;
}
