...
export const useAppSSOT = create<AppSSOTState>((set, get) => ({
  regime: 'neutral', horizon: '1h', tz: 'UTC+3', accuracyWindow: '24h', dataVersion: 'Mock API v5.2',
  setRegime: (r) => set({ regime: r }), setHorizon: (h) => set({ horizon: h }),
  syncFromUrl: (url: URL) => { const r = (url.searchParams.get('regime') as Regime) || get().regime; const h = (url.searchParams.get('h') as Horizon) || get().horizon; set({ regime: r, horizon: h }); },
}));
