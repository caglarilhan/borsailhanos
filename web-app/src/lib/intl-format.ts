...
export function clampPercent(x: number): number { if (!isFinite(x)) return 0; return Math.max(0, Math.min(100, x)); }
export function clampPP(pp: number): number { if (!isFinite(pp)) return 0; return Math.max(-10, Math.min(10, pp)); }
