...
INVESTOR_STYLES = {
    "buffett": {"name": "Warren Buffett", "tone": "uzun vadeli, temkinli", "logic": "intrinsic value, temettü", "avatar": "💎", "color": "emerald"},
    "lynch": {"name": "Peter Lynch", "tone": "büyüme odaklı", "logic": "PEG oranı, satış büyümesi", "avatar": "🚀", "color": "purple"},
    "dalio": {"name": "Ray Dalio", "tone": "makro, döngüsel", "logic": "faiz, enflasyon", "avatar": "🌍", "color": "blue"},
    "simons": {"name": "Jim Simons", "tone": "istatistiksel", "logic": "pattern tanıma, RSI", "avatar": "🧠", "color": "orange"},
    "soros": {"name": "George Soros", "tone": "riskli, spekülatif", "logic": "piyasa psikolojisi", "avatar": "⚡", "color": "red"},
    "wood": {"name": "Cathie Wood", "tone": "yenilikçi", "logic": "AI, genomik", "avatar": "🔮", "color": "pink"},
    "burry": {"name": "Michael Burry", "tone": "şüpheci", "logic": "aşırı değerlenme", "avatar": "🕳️", "color": "gray"}
}
...
async def analyze_investor(mode: str = Query("buffett"), symbol: str = Query("THYAO")):
    ...
    comments = {
        "buffett": f"{i['name']} tarzı {i['tone']} analize göre; {symbol} için yaklaşım {i['logic']} göstergelerini esas alır. Temelleri sağlam.",
        "soros": f"{i['name']} tarzı {i['tone']} yaklaşıma göre; piyasa inançtan besleniyor. Momentum pozitif.",
        "simons": f"{i['name']} model: Veri yapısı istikrarlı, pattern uyumu %85.",
        "lynch": f"{i['name']} yaklaşımı: PEG oranı karışık. Sektör trendi destekliyor.",
        "dalio": f"{i['name']} makro yaklaşımı: Risk paritesi dengeli.",
        "wood": f"{i['name']} yenilik odaklı: Teknoloji trendi güçlü.",
        "burry": f"{i['name']} şüpheci yaklaşım: Aşırı değerleme uyarısı."
    }
